...
context("CYGNUS BORROW: DEPOSIT DAI & REDEEM CYGDAI", function () {
  ...
  describe("Borrower deposits LP Token for CygLP", async () => {
    ...
    it("Deposits LP Token in collateral before approving router in LP: FAIL", async () => {
      await expect(
        router.connect(borrower).mintCollateral(collateral.address, lpTokenAmount, borrower._address, max, "0x"),
      ).to.be.reverted;
    });
  ...
  describe("Borrower takes out a DAI loan", async () => {
    ...
    it("Borrows without `borrowApprove` call: FAIL { CygnusBorrowApprove__BorrowNotAllowed }", async () => {
      await expect(router.connect(borrower).borrow(borrowable.address, BigInt(10e18), borrower._address, max, "0x")).to
        .be.reverted;
    });
    ...
    it("Borrower takes out more than his collateral available at 95% debt ratio: FAIL { Insufficient_Liquidity  }", async () => {
      ...
      await expect(
        router.connect(borrower).borrow(borrowable.address, maxLiquiditySlightlyMore, borrower._address, max, "0x"),
      ).to.be.reverted;
    });
    ...
    it("Borrower cant borrow more", async () => {
      // Borrow Min
      await expect(router.connect(borrower).borrow(borrowable.address, BigInt(1e18), borrower._address, max, "0x")).to
        .be.reverted;
    });
  ...
  describe("Borrower repays loans", function () {
    ...
    it("Borrower repays part of his loan back without approving router in DAI: FAIL { InsufficientAllowance }", async () => {
      ...
      // Static call the debt ratio
      await expect(router.connect(borrower).repay(borrowable.address, loanRepayAmount, borrower._address, max)).to.be
        .reverted;
    });
    ...
    it("Borrower repays 50% of his loan back in DAI and emits { Borrow }", async () => {
      ...
      // Static call the debt ratio
      await expect(router.connect(borrower).repay(borrowable.address, loanRepayAmount, borrower._address, max)).to.emit(
        borrowable,
        "Borrow",
      );
    });
