...
///         amount possible.
module.exports = async function openOceanLeverage(chainId, lpToken, nativeToken, usdc, router, leverageUsdcAmount, nebula) {
    // Get LP info
    ...
    const openOcean = async (fromToken, toToken, amount, router) => {
        ...
        amount = (+amount - 1) / +_scalar;

        // Api URL
        const apiUrl = `https://open-api.openocean.finance/v3/${chainId}/swap_quote?inTokenAddress=${fromToken}&outTokenAddress=${toToken}&amount=${amount}&slippage=0.5&gasPrice=${_gasPrice}&account=${router}`;
