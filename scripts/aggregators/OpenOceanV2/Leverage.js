...
module.exports = async function openOceanLeverage(chainId, lpToken, nativeToken, usdc, router, leverageUsdcAmount) {
    ...
    const openOcean = async (fromToken, toToken, amount, router) => {
        ...
        // Api URL
        const apiUrl = `https://open-api.openocean.finance/v3/${chainId}/swap_quote?inTokenAddress=${fromToken}&outTokenAddress=${toToken}&amount=${amount}&slippage=1&gasPrice=${_gasPrice}&account=${router}&disabledDexIds=33`;
