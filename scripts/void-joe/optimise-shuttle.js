...
async function deploy() {
    ...
    await router
        .connect(borrower)
        .leverage(collateral.address, BigInt(oneLPToken) * BigInt(100), borrower._address, max, '0x');
    ...
    console.log('BORROWER: Debt ratio after x9 levereage: %s', debtRatioAfter / 1e18);

    const denebBalanceAfter = await collateral.balanceOf(borrower._address);

    console.log('BORROWER: CYG-LP balance of borrower after leverage: %s CYG-LP', denebBalanceAfter / 1e18);

    const albireoBalanceAfter = await borrowable.totalBalance();

    console.log('PROTOCOL: totalBalance of borrowable after leverage: %s DAI', albireoBalanceAfter / 1e18);

    const totalBalanceAfterC = await collateral.totalBalance();

    console.log('PROTOCOL: totalBalance of collateral after leverage: %s LP TOKENS', totalBalanceAfterC / 1e18);

    console.log('DAI BALANCE OF :', DAI.balanceOf(borrower._address));
    nsole.log('DAI BALANCE OF :', DAI.balanceOf(router.address));
    nsole.log('DAI BALANCE OF :', DAI.balanceOf(borrowable.address));
    nsole.log('DAI BALANCE OF :', DAI.balanceOf(collateral.address));
}
